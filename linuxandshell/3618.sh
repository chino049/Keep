...
for f in $files; do
        ...
        if [ $file_check -eq 5 ]; then
                res=`sed -n '/3\.6\.18/,/\*\*\*\*\*/p' $f | grep 'rpc' | grep  -v '3.6.18 sp_audit (rpc)' | awk '{print $2}'`
                echo " "
                echo Processing file $f
                if [ "$res" != "on" ]; then
                        echo FAIL on $instance_name, sp_audit rpc, value is $res
                        fail="y"
                else
                        echo PASS on $instance_name, sp_audit rpc, value is $res
                        pass="y"
                fi
        fi
done
